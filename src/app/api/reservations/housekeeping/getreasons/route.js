...
export async function GET(request) {
    try {
        ...
        const url = `http://${propertyServer}:${propertyPort}/getmaintenancereasons`;
        ...
        const formattedReasons = reasons.map((reason) => ({
            value: reason.ref,       // ID do tipo de documento
            label: reason.text       // Nome do tipo de documento
        }));

        // Retornar os dados como JSON
        return new NextResponse(
            JSON.stringify(formattedReasons),
            { status: 200, headers: { "Content-Type": "application/json; charset=utf-8" } }
        );
    } catch (error) {
        console.error("Erro ao buscar razoes de manutenção:", error);
        return new NextResponse(
            JSON.stringify({ error: "Erro ao buscar razoes de manutenção." }),
