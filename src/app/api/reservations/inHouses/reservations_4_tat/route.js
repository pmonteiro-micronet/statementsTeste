...
export async function GET(request) {
  ...
  try {
    ...
    const existingRequest = await prisma.requestRecordsInHouses.findFirst({
      where: {
        propertyID: propertyIDInt,
        responseStatus: "PROCESSING", // Verifica pedidos em andamento
      },
    });
    ...
    const processingRequest = await prisma.requestRecordsInHouses.create({
      data: {
    ...
    try {
      ...
      const url = `http://${propertyServer}:${propertyPort}/pp_xml_ckit_statementinhouses`;
      ...
      // Apagar todos os registros da tabela requestRecordsInHouses antes de inserir novos dados
      await prisma.requestRecordsInHouses.deleteMany({
        where: {
          propertyID: propertyIDInt,
        },
      });
      ...
      const newRequest = await prisma.requestRecordsInHouses.create({
        data: {
          requestBody: response.data, // Armazena o array completo como JSON
      ...
      await prisma.requestRecordsInHouses.update({
