...
const API_KEY =
  ...
  export async function POST(request) {
    ...
    try {
      const { firstName, secondName, email, password, pin, propertyTags, permissions } = await request.json();
      ...
      if (typeof permissions !== "number" || permissions < 0) {
        return new Response(
          JSON.stringify({ error: "O campo permissions deve ser um número inteiro positivo." }),
          { status: 400 }
        );
      }
