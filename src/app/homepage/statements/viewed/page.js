        ...
        {uniqueJsons.length > 0 ? (
          uniqueJsons.map((json, index) => {
            ...
            try {
              ...
              return <div key={index}>Error loading data</div>; // Exibe mensagem de erro ao invés de retornar `null`
            }
            ...
            const guestFullName = guestInfo
              ? `${guestInfo.Salution || ""} ${guestInfo.LastName || ""}`
              : (translations[locale]?.statements?.view?.guestInfoNotAvailable || "Guest info not available");

            const roomNumber = reservation?.RoomNumber || t.statements.view.roomNumberNotAvailable;
            const reservationNumber = reservation?.ReservationNumber || t.statements.view.reservationNumberNotAvailable;
            const checkInDate = reservation?.DateCI || t.statements.view.checkInNotAvailable;
            const checkOutDate = reservation?.DateCO || t.statements.view.checkOutNotAvailable;
            const itemsDescription = parsedData[0]?.Items?.map(item => item.Description).join(", ") || t.statements.view.itemsNotAvailable;
            const taxes = parsedData[0]?.Taxes?.map(tax => tax.Taxes).join(", ") || t.statements.view.taxesNotAvailable;
              ...
              <div key={index} className="card" onClick={() => handleCardClick(json)}>
                ...
                <div className="flex flex-col absolute top-12 left-4">
                  <p className="text-sm text-gray-500">
                    <span className="text-lg text-gray-900 font-semibold">{guestFullName}</span>
                  </p>
                </div>
