...
const PendentesPage = () => {
  ...
  return (
    <div
      className="min-h-screen flex flex-col p-8 bg-background"
    >
  ...
  filteredJsons.map((json, index) => {
    // Parse o JSON
    let reservation, guestInfo;
    try {
      const parsedData = JSON.parse(json.requestBody);
      reservation = Array.isArray(parsedData.Reservation) ? parsedData.Reservation[0] : null;
      guestInfo = Array.isArray(parsedData.GuestInfo) ? parsedData.GuestInfo[0] : null;
    } catch (error) {
      console.error("Erro ao analisar o JSON:", error);
      return null; // Ignora este item se houver um erro de análise
    }

    // Verifica se as informações necessárias existem
    if (!reservation || !guestInfo) {
      return null; // Ignora este item se os dados não existirem
    }
